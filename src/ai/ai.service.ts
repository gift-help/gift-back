...
export class AiService {
  ...
  async generateGiftIdeas(keywords: string[]): Promise<string[]> {
    const prompt = `
      На основе характеристик человека, предложи 10 оригинальных идей подарков, которые соответствуют запросу:
      ${keywords.join(', ')}
